var onmessage = function( e ) {
	othello.board = e.data.board;
	var ret;
	switch (e.data.level) {
	case 1:
		ret = othello.minMaxWithNumber(othello, 3, e.data.color, e.data.color);
		break;
	case 2:
		ret = othello.minMaxWithPosition(othello, 3, e.data.color, e.data.color);
		break;
	case 3:
		ret = othello.minMax(othello, 3, e.data.color, e.data.color);
		break;
	};
    // 結果を返す
    postMessage(ret);
...
// OthelloBoardコンストラクタ
var OthelloBoard = function (board, pos, col) {
	// ボードの状態（一次元配列）
	this.board = board;
...
var OthelloBoardProto = {
	...
	minMaxWithNumber: function (ob, depth, self_col, thinker_col) {
		...
		// 末端深さに達したならば
		if (depth == 0) {
			return ob.evalNumber(thinker_col);
		};
	...
	minMaxWithPosition: function (ob, depth, self_col, thinker_col) {
		...
		// 末端深さに達したならば
		if (depth == 0) {
			return ob.evalPosition(thinker_col);
		};
