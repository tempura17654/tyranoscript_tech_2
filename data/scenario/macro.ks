...
; ----------------------------------------------------------
	...
	[macro name="set_default_view"]
		...
		; [iscript]～[endscript]
		; マクロに渡された属性値を一時変数にコピーします。
		; その際、初期値を設定しています。
		; ここは難しいので、わからなくても大丈夫。
		[iscript]
			;「mp.属性名」のように書くことで、マクロに渡された属性値を
			; JavaScript式で扱うことができます。
			; また、「 mp.chara || 'yes' 」のように書くことで、
			; mp.charaが何も指定されなかった場合の代わりの値（ようするに、初期値）
			; を指定することができます。
			tf.chara     = mp.chara     || 'yes'
				; ※ 余談 ※ さらに難しい話
				; そもそもなぜわざわざ一時変数にマクロの属性値をぶち込んでいるのか？
				; 「mp.」で属性値を参照できるなら、それで参照すればいいじゃないか！
				; と思われるかもしれません。
				; なぜこんなことをしているかというと、
				;「マクロ内で別のマクロを使用した場合、それ以降「mp.」での参照がうまくいかなくなるから」
				; です。
				; この場合なら、[set_default_view] というマクロのなかで [destroy] というマクロを使っています。
				; このとき、 [destroy] を使った時点で、それ以降「mp.」での参照がうまくいかなくなります。
				; その対策として、マクロの属性値の控えを一時変数に持っておく、という発想をしています。
		[endscript]
